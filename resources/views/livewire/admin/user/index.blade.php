...
new #[Layout('components.layouts.admin')] class extends Component {
    ...
    public function getFilteredUsersProperty()
    {
        return User::with(['roles']) // Eager load relationships
            ->search($this->search)
            ->latest()
...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
            <section>
                <x-action-message class="me-3" on="user-deleted" message='User Removed'/>
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse ($this->filteredUsers as $user)
                                    <tr wire:key="User-{{ $user->id }}">
                                        ...
                                        <td class="px-4 py-2 text-gray-900">{{ $user->phone }}</td>
                                        <td class="px-4 py-2 text-gray-900">
                                            {{ $user->hasRole('corporate') ? 'Corporate' : 'Customer' }}
                                        </td>
                                        <td class="px-4 py-2 text-gray-900 uppercase">
                                            {{ $user->status}}
                                        </td>
