...
new #[Layout('components.layouts.admin')] class extends Component {
    ...
    public function getFilteredCouponsProperty()
    {
        return Coupon::where('name', 'like', '%' . $this->search . '%')
...
<x-slot name="header">
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('admin.setting.coupon.create') }}"
...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
            <section>
                <x-action-message class="me-3" on="coupon-deleted" message='Coupon Removed'/>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse ($this->filteredCoupons as $coupon)
                                    <tr wire:key="Coupon-{{ $coupon->id }}">
                                        <td class="px-4 py-2 text-gray-900">{{ $coupon->name }}</td>
                                        <td class="px-4 py-2 text-gray-900">{{ $coupon->code }}</td>
                                        <td class="px-4 py-2 text-gray-900">{{ $coupon->discount_value }}%</td>
                                        <td class="px-4 py-2 text-gray-900">
                                            {{ $coupon->status ? 'Active' : 'In Active' }}
                                        </td>
                                        <td class="px-4 py-2 text-gray-700 flex gap-2">
                                            <a href="{{ route('admin.setting.coupon.edit',$coupon->uuid) }}"
                                                class="px-3 py-1 text-white bg-black rounded-md hover:bg-gray-800" wire:navigate>Edit</a>
                                            <button wire:click="destroy({{ $coupon->id }})"
