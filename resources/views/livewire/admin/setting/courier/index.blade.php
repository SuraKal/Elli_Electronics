...
new #[Layout('components.layouts.admin')] class extends Component {
    ...
    public function getFilteredCouriersProperty()
    {
        return Courier::where('name', 'like', '%' . $this->search . '%')
...
<x-slot name="header">
    <div class="flex justify-between items-center">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('List of Delivery Partners') }}
        </h2>

        <a href="{{ route('admin.setting.courier.create') }}"
...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
            <section>
                <x-action-message class="me-3" on="courier-deleted" message='Partner Removed'/>
                        ...
                        <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
                            <thead class="text-left bg-gray-100">
                                <tr>
                                    ...
                                    <th class="px-4 py-2 font-medium text-gray-900">Contact phone</th>
                                    <th class="px-4 py-2 font-medium text-gray-900">Contact email</th>
                                    <th class="px-4 py-2 font-medium text-gray-900">Status</th>
                                    <th class="px-4 py-2 font-medium text-gray-900">Featured</th>

                                    <th class="px-4 py-2 font-medium text-gray-900">Actions</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-gray-200">
                                @forelse ($this->filteredCouriers as $courier)
                                    <tr wire:key="Couriers-{{ $courier->id }}">
                                        <td class="px-4 py-2 text-gray-900">{{ $courier->name }}</td>
                                        <td class="px-4 py-2 text-gray-900">{{ $courier->contact_phone }}</td>
                                        <td class="px-4 py-2 text-gray-900">{{ $courier->contact_email }}</td>
                                        <td class="px-4 py-2 text-gray-900">
                                            {{ $courier->status ? 'Active' : 'In Active' }}
                                        </td>
                                        <td class="px-4 py-2 text-gray-900">
                                            {{ $courier->is_featured ? 'Is Featured' : '' }}
                                        </td>
                                        <td class="px-4 py-2 text-gray-700 flex gap-2">
                                            <a href="{{ route('admin.setting.courier.edit',$courier->uuid) }}"
                                                class="px-3 py-1 text-white bg-black rounded-md hover:bg-gray-800" wire:navigate>Edit</a>
                                            <button wire:click="destroy({{ $courier->id }})"
