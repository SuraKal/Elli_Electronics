...
new #[Layout('components.layouts.admin')] class extends Component {
    ...
    public function getFilteredCategorysProperty()
    {
...
<x-slot name="header">
    <div class="flex justify-between items-center">
        ...
        <a href="{{ route('admin.store.category.create') }}"
...
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
        <div class="p-4 sm:p-8 bg-white shadow sm:rounded-lg">
            <section>
                <x-action-message class="me-3" on="category-deleted" message='Category Removed'/>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
                            ...
                            <tbody class="divide-y divide-gray-200">
                                @forelse ($this->filteredCategorys as $category)
                                    <tr wire:key="Category-{{ $category->id }}">
                                        <td class="px-4 py-2 text-gray-900">{{ $category->name }}</td>
                                        <td class="px-4 py-2 text-gray-900">
                                            {{ $category->status ? 'Active' : 'In Active' }}
                                        </td>
                                        <td class="px-4 py-2 text-gray-700 flex gap-2">
                                            <a href="{{ route('admin.store.category.edit', parameters: $category->uuid) }}"
